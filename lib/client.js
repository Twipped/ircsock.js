...
Client.prototype.connect = function (options, fn) {
	...
	if (!this._validateOptions()) {
		var e = new Error('One or more required connection details were missing.');
		self.emit('error', e);
		if (fn) {fn(e);}
		return;
	}
...
Client.prototype.sendHandshake = function () {
...
Client.prototype.registerDefaultHandlers = function () {
	...
	var self = this;
	this.on('RPL_ISUPPORT', function (options) {
		if (options.CHANTYPES) {
			var prefixes = options.CHANTYPES.split('');
			self.isTargetChannel = function (target) {
				return prefixes.indexOf(String(target)[0]) > -1;
			};
		}
	});
};

Client.prototype.isTargetChannel = function (target) {
